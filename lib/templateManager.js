...
function renderDust(source, locals, cb) {
  ...
  if (!dust.cache[dm]) {
    // Recursive compile
    dustPartial({file: locals.filename, name: dm}, function() {
  ...
  function dustPartial(file, cb) {
    fs.readFile(file.file, { encoding: 'utf-8' }, function(err, source) {
      ...
      var match;
      while (match = partial_re.exec(source)) {
        sources.push({
          file: path.join(dir, match[1] + '.dust'),
          name: match[1]
        });
      }
      async.map(sources, dustPartial, function() {
        if (!dust.cache[file.name]) {
          var c = dust.compile(source, file.name);
          dust.loadSource(c);
        }
        cb();
      });
