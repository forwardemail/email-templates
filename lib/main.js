  ...
  fs.stat(templateDirectory, function(err, stats) {
    ...
    if (!stats.isDirectory())
      ...
      if(typeof templateName === 'object') {
        // We are submitting our own html, stylesheet, and text parameters, not from a file.
        templatePath = scopeInfo.templatePath = path.join(templateDirectory, templateName.template);
        scopeInfo.html = templateName.html || '';
        scopeInfo.stylesheet = templateName.stylesheet || false;
        scopeInfo.text = templateName.text || false;
        batchCheck();
      } else {
        templatePath = scopeInfo.templatePath = path.join(templateDirectory, templateName);
      }
